...
class ServerStatusConditionTest {
    ...
    class Factory {
        ...
        void createSucceeds() {
            ConditionContext ctx = ConditionContext.builder()
                    .serverManager(serverManager).variableResolver(resolver).build();
            ServerStatusCondition condition = ServerStatusCondition.create(
                    Map.of("server", "lobby", "status", "online"), ctx);
    ...
    void getTypeReturnsServerStatus() {
        ServerStatusCondition condition = new ServerStatusCondition(
                "lobby", "online", serverManager, resolver);
    ...
    class OfflineChecks {
        ...
        void trueWhenOfflineExpectedAndOffline() {
            when(serverManager.isServerOnline("lobby")).thenReturn(false);
            ServerStatusCondition condition = new ServerStatusCondition(
                    "lobby", "offline", serverManager, resolver);
            assertTrue(condition.evaluate(context));
        }
    ...
    class UnknownStatus {
        ...
        void falseForUnknownStatus() {
            when(serverManager.isServerOnline("lobby")).thenReturn(true);
            ServerStatusCondition condition = new ServerStatusCondition(
                    "lobby", "starting", serverManager, resolver);
            assertFalse(condition.evaluate(context));
        }
    }
    ...
    class VariableResolution {
        ...
        void resolvesServerName() {
            ...
            ServerStatusCondition condition = new ServerStatusCondition(
                    "${target}", "online", serverManager, resolver);
            assertTrue(condition.evaluate(context));
        ...
        void resolvesStatus() {
            ...
            ServerStatusCondition condition = new ServerStatusCondition(
                    "lobby", "${expected_status}", serverManager, resolver);
            assertTrue(condition.evaluate(context));
        }
        ...
        void resolvesBoth() {
            ...
            ServerStatusCondition condition = new ServerStatusCondition(
                    "${srv}", "${sts}", serverManager, resolver);
            assertTrue(condition.evaluate(context));
