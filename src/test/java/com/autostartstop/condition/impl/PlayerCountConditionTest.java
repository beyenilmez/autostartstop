...
class PlayerCountConditionTest {
    ...
    class Factory {
        ...
        void createThrowsWhenServerMissing() {
            ConditionContext ctx = ConditionContext.builder()
                    .serverManager(serverManager).variableResolver(resolver).build();
            assertThrows(ConfigException.class,
                    () -> PlayerCountCondition.create(Map.of(), ctx));
        }
        ...
        void createSucceedsWithServerOnly() {
            ConditionContext ctx = ConditionContext.builder()
                    .serverManager(serverManager).variableResolver(resolver).build();
            PlayerCountCondition condition = PlayerCountCondition.create(
                    Map.of("server", "lobby"), ctx);
    ...
    void getTypeReturnsPlayerCount() {
        PlayerCountCondition condition = new PlayerCountCondition(
                "lobby", null, null, null, serverManager, resolver);
    ...
    class EqualsConstraint {
        ...
        void trueWhenMatches() {
            ...
            PlayerCountCondition condition = new PlayerCountCondition(
                    "lobby", null, null, "10", serverManager, resolver);
        ...
        void equalsTakesPriority() {
            ...
            PlayerCountCondition condition = new PlayerCountCondition(
                    "lobby", "0", "100", "5", serverManager, resolver);
            assertFalse(condition.evaluate(context));
        }
        ...
        void equalsZero() {
            ...
            PlayerCountCondition condition = new PlayerCountCondition(
                    "lobby", null, null, "0", serverManager, resolver);
            assertTrue(condition.evaluate(context));
        }
    }
    ...
    class MinConstraint {
        ...
        void trueWhenAboveMin() {
            when(serverManager.getServerPlayerCount("lobby")).thenReturn(10);
            PlayerCountCondition condition = new PlayerCountCondition(
                    "lobby", "5", null, null, serverManager, resolver);
            assertTrue(condition.evaluate(context));
        }
    ...
    class MaxConstraint {
        ...
        void trueWhenBelowMax() {
            when(serverManager.getServerPlayerCount("lobby")).thenReturn(10);
            PlayerCountCondition condition = new PlayerCountCondition(
                    "lobby", null, "20", null, serverManager, resolver);
            assertTrue(condition.evaluate(context));
        }
    ...
    class RangeConstraint {
        ...
        void trueWhenWithinRange() {
            when(serverManager.getServerPlayerCount("lobby")).thenReturn(10);
            PlayerCountCondition condition = new PlayerCountCondition(
                    "lobby", "5", "20", null, serverManager, resolver);
            assertTrue(condition.evaluate(context));
        }
        ...
        void trueAtBoundaries() {
            // At min boundary
            when(serverManager.getServerPlayerCount("lobby")).thenReturn(5);
            PlayerCountCondition atMin = new PlayerCountCondition(
                    "lobby", "5", "20", null, serverManager, resolver);
            assertTrue(atMin.evaluate(context), "should pass at min boundary");

            // At max boundary
            when(serverManager.getServerPlayerCount("lobby")).thenReturn(20);
            PlayerCountCondition atMax = new PlayerCountCondition(
                    "lobby", "5", "20", null, serverManager, resolver);
    ...
    class VariableResolution {
        ...
        void resolvesServerName() {
            ...
            PlayerCountCondition condition = new PlayerCountCondition(
                    "${target_server}", "5", null, null, serverManager, resolver);
            assertTrue(condition.evaluate(context));
        ...
        void resolvesMinFromVariable() {
            ...
            PlayerCountCondition condition = new PlayerCountCondition(
                    "lobby", "${min_players}", null, null, serverManager, resolver);
            assertTrue(condition.evaluate(context));
        }
        ...
        void resolvesMaxFromVariable() {
            ...
            PlayerCountCondition condition = new PlayerCountCondition(
                    "lobby", null, "${max_players}", null, serverManager, resolver);
            assertFalse(condition.evaluate(context));
        }
        ...
        void resolvesEqualsFromVariable() {
            ...
            PlayerCountCondition condition = new PlayerCountCondition(
                    "lobby", null, null, "${exact_count}", serverManager, resolver);
            assertTrue(condition.evaluate(context));
        }
    }
}
