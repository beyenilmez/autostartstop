...
class ConnectionTriggerTest {
    ...
    private ConnectionTrigger createTrigger(TriggerConfig.ServerListConfig serverList,
                                             TriggerConfig.PlayerListConfig playerList,
                                             boolean denyConnection) {
        return new ConnectionTrigger(proxy, plugin, serverManager, serverList, playerList, denyConnection);
    }

    private void activateTrigger(ConnectionTrigger trigger,
                                  Function<ExecutionContext, CompletableFuture<Void>> callback) {
        trigger.activate("test-rule", callback);
    }

    private ServerPreConnectEvent mockConnectEvent(String serverName, String playerName, UUID playerUuid) {
    ...
    class ServerListFiltering {
        ...
        void defaultsToWhitelistWhenModeNull() {
            ...
            TriggerConfig.ServerListConfig serverList = new TriggerConfig.ServerListConfig(
                    java.util.Map.of("servers", List.of("lobby")));
    ...
    class PlayerListFiltering {
        ...
        void whitelistAllowsMatchingPlayer() {
            TriggerConfig.PlayerListConfig playerList = new TriggerConfig.PlayerListConfig(
    ...
    class DenyConnection {
        ...
        void createsReleaseSignalWhenDeny() {
            ...
            ExecutionContext ctx = captured.get();
            assertNotNull(ctx);
            assertTrue(ctx.hasEventReleaseSignal(), "should create release signal when deny_connection=true");
        }
    }
