 ...
 * context: - ${ping} - ProxyPingEvent object - ${ping.server} - Backend server name (resolved from
 * virtual host, null if no match) - ${ping.server.version_name} - Server version name -
 * ${ping.server.protocol_version} - Server protocol version - ${ping.server.player_count} - Current
 * player count - ${ping.server.max_players} - Maximum players - ${ping.server.motd} - Server MOTD
 * (in MiniMessage format) - ${ping.player.remote_address} - Client remote address -
 * ${ping.player.virtual_host} - Client virtual host - ${ping.player.protocol_version} - Client
 * protocol version
 */
public class PingTrigger implements Trigger {
  ...
  // Merged virtual hosts from both virtual_host_list and server_list (computed once during
  // activate)
  ...
  public void activate(
    ...
    if (virtualHostList != null
        && virtualHostList.getVirtualHosts() != null
        && !virtualHostList.getVirtualHosts().isEmpty()) {
    ...
    if (virtualHostListMode != null && serverListMode != null) {
      if (virtualHostListMode.equalsIgnoreCase(serverListMode)) {
        ...
        logger.warn(
            "PingTrigger: conflicting filter modes (virtual_host_list: {}, server_list: {}), using whitelist",
            virtualHostListMode,
            serverListMode);
      }
    ...
    if (!mergedVirtualHosts.isEmpty()) {
      logger.debug(
          "PingTrigger: merged {} virtual hosts (mode: {})", mergedVirtualHosts.size(), mergedMode);
    }
    ...
    if (!virtualHostToServerMap.isEmpty()) {
      logger.debug(
          "PingTrigger: built virtual_host -> server map with {} entries",
          virtualHostToServerMap.size());
    }

    String virtualHostFilterInfo =
        virtualHostList != null && virtualHostList.getVirtualHosts() != null
    ...
    String serverFilterInfo =
        serverList != null && serverList.getServers() != null
            ...
            : "none";
    logger.debug(
        "PingTrigger: activating for rule '{}' (virtual_host filter: {}, server filter: {}, hold_response: {})",
        ruleName,
        virtualHostFilterInfo,
        serverFilterInfo,
        holdResponse);
  ...
  public void deactivate() {
    ...
    logger.debug(
        "PingTrigger: deactivating for rule '{}'", ruleName != null ? ruleName : "unknown");
  ...
  public void onProxyPing(ProxyPingEvent event) {
    ...
    if (holdResponse && executionFuture != null) {
      logger.debug(
          "PingTrigger: waiting for rule execution or release signal (hold_response=true)");
      try {
        ...
        logger.error(
            "PingTrigger: error waiting for rule execution for '{}': {}", ruleName, e.getMessage());
  ...
  private void emitContext(
    ...
    String serverName =
        domainUsed != null && virtualHostToServerMap != null
  ...
  /**
   * Builds virtual_host (lowercase) -> server name map from plugin configuration. Called once in
   * activate() so ping.server resolution is O(1).
   */
  ...
  /**
   * Gets virtual hosts for the given server names from the plugin configuration. Returns a set of
   * lowercase virtual host strings for matching.
   */
    ...
    for (String serverName : serverNames) {
      ...
      if (serverConfig != null) {
        ...
        if (virtualHost != null && !virtualHost.isBlank()) {
          ...
          logger.debug(
              "PingTrigger: mapped server '{}' to virtual_host '{}'", serverName, virtualHost);
        } else {
