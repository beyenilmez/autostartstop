 ...
 * hybrid approach: - Event-driven: listens to player connect/disconnect events and schedules timers
 * - Periodic check: catches servers that started after activation without any player activity
 *
 * <p>Configuration: - empty_time: Duration the server must be empty before triggering (default:
 * 15m) - server_list: Optional whitelist/blacklist of servers to monitor
 *
 * <p>Emitted context: - ${empty_server.server} - RegisteredServer object -
 * ${empty_server.server.name} - Server name - ${empty_server.empty_time} - Configured empty time -
 * ${empty_server.empty_since} - ISO-8601 timestamp when server became empty
 */
public class EmptyServerTrigger implements Trigger {
  private static final Logger logger = PluginLogger.get(EmptyServerTrigger.class);
  ...
  public static EmptyServerTrigger create(TriggerConfig config, TriggerContext context) {
    ...
    if (settings != null && settings.getEmptyServerCheckInterval() != null) {
      try {
        ...
        logger.warn(
            "EmptyServerTrigger: invalid empty_server_check_interval '{}', using default {}",
            settings.getEmptyServerCheckInterval(),
            DurationUtil.format(DEFAULT_CHECK_INTERVAL));
      }
    }

    return new EmptyServerTrigger(
        context.proxy(),
        context.plugin(),
  ...
  public void activate(
      String ruleName, Function<ExecutionContext, CompletableFuture<Void>> executionCallback) {
    this.ruleName = ruleName;
    this.executionCallback = executionCallback;
    this.activated = true;

    String serverFilterInfo =
        serverList != null && serverList.getServers() != null
    ...
    logger.debug(
        ...
        ruleName,
        DurationUtil.format(emptyTime),
        DurationUtil.format(checkInterval),
        serverFilterInfo);
    ...
    logger.debug(
        "EmptyServerTrigger: registered for DisconnectEvent, ServerConnectedEvent, and periodic check");
  }
  ...
  public void deactivate() {
    ...
    logger.debug(
        "EmptyServerTrigger: deactivating for rule '{}'", ruleName != null ? ruleName : "unknown");
    ...
    synchronized (lock) {
      for (Map.Entry<String, ScheduledTask> entry : pendingEmptyTasks.entrySet()) {
        ...
        logger.debug(
            "EmptyServerTrigger: cancelled pending empty task for server '{}'", entry.getKey());
      }
  ...
  private void startPeriodicCheck() {
    ...
    logger.debug(
        "EmptyServerTrigger: started periodic check every {}", DurationUtil.format(checkInterval));
  }

  /**
   * Checks all servers and schedules timers for any empty servers not already tracked. This
   * catches: - Servers that were empty on activation - Servers that started after activation
   * without any player activity
   */
  private void checkServersForEmpty() {
    ...
    for (RegisteredServer server : proxy.getAllServers()) {
      ...
      if (playerCount == 0 && isServerOnline(serverName)) {
        logger.debug(
            "EmptyServerTrigger: server '{}' detected as empty and online, scheduling timer",
            serverName);
  ...
  public void onDisconnect(DisconnectEvent event) {
    ...
    Optional<RegisteredServer> currentServerOpt =
        player.getCurrentServer().map(conn -> conn.getServer());
    ...
    logger.debug(
        "EmptyServerTrigger: player '{}' disconnecting from '{}' ({} players will remain)",
        player.getUsername(),
        serverName,
        remainingPlayers);
  ...
  public void onServerConnected(ServerConnectedEvent event) {
    ...
    if (previousServerOpt.isPresent()) {
      ...
      if (isServerMonitored(previousServerName)) {
        ...
        logger.debug(
            "EmptyServerTrigger: player '{}' left '{}' for '{}' ({} players remain on previous)",
            player.getUsername(),
            previousServerName,
            serverName,
            remainingPlayers);
  ...
  private void scheduleEmptyTimer(String serverName, RegisteredServer server) {
    synchronized (lock) {
      ...
      logger.debug(
          "EmptyServerTrigger: scheduling empty timer for server '{}' (will fire in {})",
          serverName,
          DurationUtil.format(emptyTime));
  ...
  private void cancelEmptyTimer(String serverName) {
    synchronized (lock) {
      ...
      if (task != null) {
        ...
        firedServers.remove(serverName);
        logger.debug(
            "EmptyServerTrigger: cancelled empty timer for server '{}' (player joined)",
            serverName);
      }
    }
  }
  ...
  private void fireEmptyTrigger(String serverName, RegisteredServer server, Instant emptySince) {
    ...
    if (currentPlayers > 0) {
      logger.debug(
          "EmptyServerTrigger: empty timer fired for '{}' but server now has {} players, ignoring",
          serverName,
          currentPlayers);
      synchronized (lock) {
    ...
    logger.debug(
        "EmptyServerTrigger: server '{}' has been empty for {}, firing trigger for rule '{}'",
        serverName,
        DurationUtil.format(emptyTime),
        ruleName);
    ...
    try {
      ...
      logger.error(
          "EmptyServerTrigger: error during execution callback for rule '{}': {}",
          ruleName,
          e.getMessage());
  ...
  private boolean isServerMonitored(String serverName) {
    if (serverList == null
        || serverList.getServers() == null
        || serverList.getServers().isEmpty()) {
