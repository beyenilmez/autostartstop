 ...
 * UNIX (default), QUARTZ, CRON4J, SPRING, SPRING53.
 *
 * <p>Configuration: - expression: The cron expression (required). - time_zone: The time zone for
 * the schedule (optional, defaults to UTC). - format: The cron format (optional, defaults to UNIX).
 * Valid values: UNIX, QUARTZ, CRON4J, SPRING, SPRING53.
 *
 * <p>Emitted context: - ${cron.expression} - The cron expression - ${cron.format} - The cron format
 * used - ${cron.time_zone} - The time zone used - ${cron.scheduled_time} - The scheduled execution
 * time (ISO-8601) - ${cron.actual_time} - The actual execution time (ISO-8601)
 */
public class CronTrigger implements Trigger {
  private static final Logger logger = PluginLogger.get(CronTrigger.class);
  ...
  public CronTrigger(
      ProxyServer proxy, Object plugin, String expression, String timeZoneStr, String formatStr) {
    this.proxy = proxy;
    this.plugin = plugin;
  ...
  public void activate(
    ...
    logger.debug(
        "CronTrigger: activating for rule '{}' with expression '{}'", ruleName, expression);

    // Parse the cron expression
    if (!parseCronExpression()) {
      logger.error(
          "CronTrigger: failed to parse cron expression '{}' for rule '{}'", expression, ruleName);
      return;
    }
    ...
    this.activated = true;
    logger.info(
        "CronTrigger: activated for rule '{}' - {} cron: '{}', next execution: {}",
        ruleName,
        cronType.name(),
        expression,
        nextScheduledTime);
  ...
  public void deactivate() {
    ...
    logger.debug(
        "CronTrigger: deactivating for rule '{}'", ruleName != null ? ruleName : "unknown");
  ...
  private boolean parseCronExpression() {
    ...
    try {
      ...
      logger.error(
          "CronTrigger: expression '{}' is not a valid {} cron expression: {}",
          expression,
          cronType.name(),
          e.getMessage());
      return false;
  ...
  private boolean scheduleNextExecution() {
    ...
    if (nextExecutionOpt.isEmpty()) {
      logger.warn(
          "CronTrigger: could not determine next execution time for rule '{}' with cron '{}'",
          ruleName,
          expression);
      return false;
    }
    ...
    if (delay.isNegative() || delay.isZero()) {
      ...
      logger.debug(
          "CronTrigger: delay is non-positive, firing immediately for rule '{}'", ruleName);
      fireAndReschedule();
      return true;
    }

    logger.debug(
        "CronTrigger: scheduling rule '{}' to execute at {} (in {})",
        ruleName,
        nextExecution,
        DurationUtil.format(delay));

    synchronized (scheduleLock) {
      scheduledTask =
          proxy.getScheduler().buildTask(plugin, this::fireAndReschedule).delay(delay).schedule();
    }
  ...
  private void fireAndReschedule() {
    ...
    logger.debug(
        "CronTrigger: firing for rule '{}' (scheduled: {}, actual: {})",
        ruleName,
        scheduledTime,
        actualTime);
    ...
    try {
      ...
      logger.error(
          "CronTrigger: error during execution callback for rule '{}': {}",
          ruleName,
          e.getMessage());
    ...
    if (activated) {
      ...
      logger.debug(
          "CronTrigger: rescheduling rule '{}' to execute at {} (in {})",
          ruleName,
          nextExecution,
          DurationUtil.format(delay));
