...
public class ProxyStartTrigger implements Trigger {
  private static final Logger logger = PluginLogger.get(ProxyStartTrigger.class);
  ...
  public void activate(
    ...
    if (triggerContext.isReload()) {
      logger.debug(
          "ProxyStartTrigger: skipping fire for rule '{}' (this is a reload, not initial startup)",
          ruleName);
      return;
    }
    ...
    logger.debug(
        "ProxyStartTrigger: firing immediately for rule '{}' (proxy already running)", ruleName);
    fire();
  }
  ...
  public void deactivate() {
    ...
    logger.debug(
        "ProxyStartTrigger: deactivating for rule '{}'", ruleName != null ? ruleName : "unknown");
