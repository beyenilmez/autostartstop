 ...
 * registers for ServerPreConnectEvent and invokes callback when triggered. Emitted context: -
 * ${connection.player} - Player object - ${connection.player.name} - Username -
 * ${connection.player.uuid} - UUID - ${connection.server} - RegisteredServer object -
 * ${connection.server.name} - Server name - ${connection.server.status} - Server status
 * (online/offline) - ${connection.server.player_count} - Number of players on server -
...
public class ConnectionTrigger implements Trigger {
  ...
  public void activate(
    ...
    String serverFilterInfo =
        serverList != null && serverList.getServers() != null
            ...
            : "none";
    logger.debug(
        "ConnectionTrigger: activating for rule '{}' (server filter: {})",
        ruleName,
        serverFilterInfo);
  ...
  public void deactivate() {
    ...
    logger.debug(
        "ConnectionTrigger: deactivating for rule '{}'", ruleName != null ? ruleName : "unknown");
  ...
  public void onServerPreConnect(ServerPreConnectEvent event) {
    ...
    if (playerList != null
        && playerList.getPlayers() != null
        && !playerList.getPlayers().isEmpty()) {
    ...
    logger.debug(
        "ConnectionTrigger: player '{}' connecting to '{}' matched rule '{}'",
        playerName,
        serverName,
        ruleName);
    ...
    if (denyConnection) {
      logger.debug(
          "ConnectionTrigger: denying connection for player '{}' to '{}' (deny_connection=true)",
          playerName,
          serverName);
    ...
    if (denyConnection && executionFuture != null) {
      logger.debug(
          "ConnectionTrigger: waiting for rule execution or release signal (deny_connection=true)");
      try {
        ...
        if (releaseSignal != null && releaseSignal.isDone() && !executionFuture.isDone()) {
          logger.debug(
              "ConnectionTrigger: connection released early by action for rule '{}'", ruleName);
        } else {
          logger.debug("ConnectionTrigger: rule execution completed for '{}'", ruleName);
        }
      } catch (Exception e) {
        logger.error(
            "ConnectionTrigger: error waiting for rule execution for '{}': {}",
            ruleName,
            e.getMessage());
