...
public class MetricsManager {
  private static final Logger logger = PluginLogger.get(MetricsManager.class);
  ...
  public MetricsManager(
      Object plugin, Metrics.Factory metricsFactory, Supplier<PluginConfig> configSupplier) {
    ...
    metrics.addCustomChart(
        new SingleLineChart(
            ...
            () -> {
              ...
              return (int)
                  config.getRules().values().stream().filter(RuleConfig::isEnabled).count();
            }));
            ...
            () -> {
              ...
              for (RuleConfig rule : config.getRules().values()) {
                ...
                if ("start_on_connection".equals(rule.getTemplate())) {
                  if (rule.getTemplateConfig() != null
                      && rule.getTemplateConfig().getRawConfig() != null) {
