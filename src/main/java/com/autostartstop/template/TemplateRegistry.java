 ...
 * defined in TemplateType.
 */
public class TemplateRegistry {

  private static final Logger logger = PluginLogger.get(TemplateRegistry.class);
  ...
  public Template create(TemplateConfig config) {
    ...
    if (type == null) {
      logger.error(
          "Unknown template type '{}' - valid types: {}", configType, TemplateType.getValidNames());
      return null;
    }
    ...
    try {
      ...
      if (templateContext == null) {
        logger.error(
            "TemplateContext is not set - cannot create template '{}'", type.getConfigName());
        return null;
      }
