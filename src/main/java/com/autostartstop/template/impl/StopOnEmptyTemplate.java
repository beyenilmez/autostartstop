 ...
 *
 * <p>Configuration: - empty_time: Duration the server must be empty before stopping (default: 15m)
...
public class StopOnEmptyTemplate implements Template {
  private static final Logger logger = PluginLogger.get(StopOnEmptyTemplate.class);
  ...
  public void activate(String ruleName) {
    ...
    logger.debug(
        "StopOnEmptyTemplate: activating for rule '{}' (empty_time: {}, servers: {})",
        ruleName,
    ...
    if (context.settings() != null && context.settings().getEmptyServerCheckInterval() != null) {
      try {
        ...
        logger.warn(
            "StopOnEmptyTemplate: invalid empty_server_check_interval '{}', using default {}",
            context.settings().getEmptyServerCheckInterval(),
    ...
    Function<ExecutionContext, CompletableFuture<Void>> executionCallback =
        context -> {
          ...
          if (serverNameObj == null) {
            logger.warn(
                "StopOnEmptyTemplate: empty_server.server.name not found in context, cannot stop server");
            return CompletableFuture.completedFuture(null);
          }
          String serverName = serverNameObj.toString();

          logger.debug(
              "StopOnEmptyTemplate: server '{}' has been empty for {}, stopping...",
              serverName,
              DurationUtil.format(emptyTime));
