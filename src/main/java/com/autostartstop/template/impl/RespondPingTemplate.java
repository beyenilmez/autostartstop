...
public class RespondPingTemplate implements Template {
  private static final Logger logger = PluginLogger.get(RespondPingTemplate.class);
  ...
  public static RespondPingTemplate create(TemplateConfig config, TemplateContext context) {
    ...
    if ((virtualHosts == null || virtualHosts.isEmpty())
      ...
      throw new IllegalArgumentException(
          "respond_ping template requires at least one of 'virtual_hosts' or 'servers'");
    }
    ...
    if (offlineConfig == null && onlineConfig == null) {
      throw new IllegalArgumentException(
          "respond_ping template requires at least one of 'offline' or 'online' configuration");
    }

    return new RespondPingTemplate(context, virtualHosts, servers, offlineConfig, onlineConfig);
  }

  private RespondPingTemplate(
  ...
  public void activate(String ruleName) {
    ...
    logger.debug(
        "RespondPingTemplate: activating for rule '{}' (virtual_hosts: {}, servers: {})",
        ruleName,
        virtualHosts,
        servers);
    ...
    Function<ExecutionContext, CompletableFuture<Void>> executionCallback = ctx -> handlePing(ctx);
  ...
  private CompletableFuture<Void> handlePing(ExecutionContext ctx) {
    ...
    logger.debug(
        "RespondPingTemplate: server '{}' is {}", serverName, isOnline ? "ONLINE" : "OFFLINE");
  ...
  private CompletableFuture<Void> executeRespondPingAction(
      PingConfig config, ExecutionContext ctx) {
  ...
  private static class OfflineConfig implements PingConfig {
    ...
    OfflineConfig(Map<String, Object> config) {
      ...
      this.useCachedMotd =
          accessor.hasKey("use_cached_motd") ? accessor.getBoolean("use_cached_motd", false) : null;
      this.useBackendMotd =
          accessor.hasKey("use_backend_motd")
              ? accessor.getBoolean("use_backend_motd", false)
              : null;
