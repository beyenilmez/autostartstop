 ...
 *
 * <p>Configuration: - servers: List of server names to start on proxy start
 *
...
public class StartOnProxyStartTemplate implements Template {
  ...
  public static StartOnProxyStartTemplate create(TemplateConfig config, TemplateContext context) {
    ...
    if (servers == null || servers.isEmpty()) {
      throw new IllegalArgumentException(
          "start_on_proxy_start template requires at least one server");
    }
  ...
  public void activate(String ruleName) {
    ...
    logger.debug(
        "StartOnProxyStartTemplate: activating for rule '{}' (servers: {})", ruleName, servers);

    // Create the trigger directly using TriggerContext from registry
    trigger = ProxyStartTrigger.create(null, context.triggerRegistry().getTriggerContext());
    ...
    Function<ExecutionContext, CompletableFuture<Void>> executionCallback =
        context -> {
          ...
          logger.debug(
              "StartOnProxyStartTemplate: proxy start detected, starting {} server(s)...",
              servers.size());
          ...
          return allStarted.thenAccept(
              v ->
                  logger.debug(
                      "StartOnProxyStartTemplate: all {} server(s) started", servers.size()));
        };
