 ...
 *
 * <p>Configuration: - servers: List of server names to stop on proxy shutdown
 *
...
public class StopOnProxyShutdownTemplate implements Template {
  private static final Logger logger = PluginLogger.get(StopOnProxyShutdownTemplate.class);
  ...
  public static StopOnProxyShutdownTemplate create(TemplateConfig config, TemplateContext context) {
    ...
    if (servers == null || servers.isEmpty()) {
      throw new IllegalArgumentException(
          "stop_on_proxy_shutdown template requires at least one server");
    }
  ...
  public void activate(String ruleName) {
    ...
    logger.debug(
        "StopOnProxyShutdownTemplate: activating for rule '{}' (servers: {})", ruleName, servers);

    // Create the trigger directly
    trigger = new ProxyShutdownTrigger(context.proxy(), context.plugin());
    ...
    Function<ExecutionContext, CompletableFuture<Void>> executionCallback =
        ctx -> {
          ...
          logger.debug(
              "StopOnProxyShutdownTemplate: proxy shutdown detected, stopping {} server(s)...",
              servers.size());
          ...
          CompletableFuture<Void> result =
              allStopped.thenAccept(
                  v ->
                      logger.debug(
                          "StopOnProxyShutdownTemplate: all {} server(s) stopped", servers.size()));
          ...
          if (context.settings() != null) {
            try {
              ...
              logger.warn(
                  "StopOnProxyShutdownTemplate: invalid shutdown_timeout in config, using default 30s");
            }
          }
          ...
          try {
            logger.debug(
                "StopOnProxyShutdownTemplate: waiting up to {} for all servers to stop...",
                DurationUtil.format(shutdownTimeout));
            result.get(shutdownTimeout.toMillis(), TimeUnit.MILLISECONDS);
            logger.debug(
                "StopOnProxyShutdownTemplate: all actions completed for rule '{}'", ruleName);
          } catch (java.util.concurrent.TimeoutException e) {
            logger.warn(
                "StopOnProxyShutdownTemplate: timeout waiting for servers to stop for rule '{}' (timeout: {})",
                ruleName,
                DurationUtil.format(shutdownTimeout));
          } catch (Exception e) {
            logger.error(
                "StopOnProxyShutdownTemplate: error waiting for servers to stop for rule '{}': {}",
                ruleName,
                e.getMessage());
