...
public class ConfigMerger {
  private static final Logger logger = PluginLogger.get(ConfigMerger.class);
  ...
  public ServerConfig mergeServerConfig(ServerConfig specific, ServerConfig defaults) {
    ...
    merged.setStartupTimer(
        mergeStartupTimerConfig(specific.getStartupTimer(), defaults.getStartupTimer()));
  ...
  private ControlApiConfig mergeControlApiConfig(
      ControlApiConfig specific, ControlApiConfig defaults) {
    if (defaults == null) return specific;
  ...
  private StartupTimerConfig mergeStartupTimerConfig(
      StartupTimerConfig specific, StartupTimerConfig defaults) {
    if (defaults == null) return specific;
    if (specific == null) return cloneStartupTimerConfig(defaults);

    StartupTimerConfig merged = new StartupTimerConfig();
    merged.setExpectedStartupTime(
        coalesce(specific.getExpectedStartupTime(), defaults.getExpectedStartupTime()));
    merged.setAutoCalculateExpectedStartupTime(
        coalesce(
            specific.isAutoCalculateExpectedStartupTime(),
            defaults.isAutoCalculateExpectedStartupTime()));
    return merged;
  }
