 ...
 * reduce code duplication in ConfigLoader.
 */
public final class ConfigParser {
  ...
  /**
   * Parses an action configuration from a map. The action type is the first key in the map.
   *
  ...
  /**
   * Parses a trigger configuration from a map. The trigger type is the first key in the map.
   *
  ...
  public static RuleConfig parseRule(Section section, String name) {
    ...
    if (templateName != null && !templateName.isEmpty()) {
      ...
      boolean hasTriggers =
          section.getMapList("triggers") != null && !section.getMapList("triggers").isEmpty();
      boolean hasActions =
          section.getMapList("action") != null && !section.getMapList("action").isEmpty();
      boolean hasConditions = section.getSection("conditions") != null;

      if (hasTriggers || hasActions || hasConditions) {
        logger.warn(
            "Rule '{}': template rules cannot have triggers, actions, or conditions. "
                + "Ignoring triggers/actions/conditions.",
            name);
      }
