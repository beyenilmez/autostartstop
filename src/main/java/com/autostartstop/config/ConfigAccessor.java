...
/**
 * Utility class for accessing configuration values from a raw config map. Provides type-safe
 * parsing methods with default values.
 *
 * <p>Can be used as a delegate or inherited by config classes that need to parse values from a raw
 * configuration map.
 */
public class ConfigAccessor {
  ...
  /**
   * Gets a list of strings. Handles both List and single String values.
   *
   * @param key The key to look up
  ...
  /**
   * Gets a duration value. Supports duration strings like "1s", "500ms", "2m", "10t" (ticks).
   *
   * @param key The key to look up
  ...
  public Duration requireDuration(String key) {
    ...
    try {
      ...
      throw ConfigException.invalid(
          componentName, key, value, "a valid duration (e.g., '1s', '500ms', '2m')");
    }
  }
