...
/**
 * Represents an isolated execution context for a rule execution. Each trigger event creates an
 * independent context that holds variables and state for that specific execution.
 */
public class ExecutionContext {
  ...
  /**
   * Signal that can be completed by actions (like allow_connection, allow_ping) to indicate that
   * the event should be released immediately, without waiting for all actions to complete. This is
   * lazily initialized only when needed. Used for both connection and ping events.
   */
  ...
  /**
   * Gets or creates the release signal. This signal can be completed by actions (like
   * allow_connection, allow_ping) to indicate that the event should be released immediately. Used
   * for both connection and ping events.
   *
  ...
  /**
   * Completes the release signal, indicating that the event should be released immediately. Safe to
   * call even if no signal exists. Used for both connection and ping events.
   */
