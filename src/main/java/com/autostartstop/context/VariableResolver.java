 ...
 *
 * <ul>
 *   <li>Context variables set during rule execution
 *   <li>Global server variables: ${server-name.status}, ${server-name.player_count}, etc.
 *   <li>Type-safe resolution: resolveDuration(), resolveInt(), resolveEnum(), etc.
 * </ul>
 */
public class VariableResolver {
  ...
  /**
   * Extracts the variable name from ${...} syntax. Returns the input unchanged if not wrapped in
   * ${}.
   */
  ...
  /**
   * Resolves all ${...} placeholders in the input string. Supports nested variables like
   * ${${connection.server.name}.property}. Unresolved placeholders are kept as-is.
   */
  ...
  /**
   * Performs a single pass of variable resolution. Resolves innermost variables first to support
   * nesting.
   */
  ...
  /** Resolves a single variable by name. Checks context first, then global server variables. */
  ...
  public float resolveFloatClamped(
      Object raw, ExecutionContext context, float defaultValue, float min, float max) {
  ...
  public <T extends Enum<T>> T resolveEnum(
      String raw, ExecutionContext context, Class<T> enumClass, T defaultValue) {
    if (raw == null || raw.isBlank()) {
      return defaultValue;
    }
  ...
  public List<String> resolveList(List<String> raw, ExecutionContext context) {
    ...
    return raw.stream().map(s -> resolve(s, context)).toList();
  }
   ...
   * <ul>
   *   <li>.name - Server name
   *   <li>.status - "online" or "offline"
   *   <li>.state - Detailed state (offline/starting/online)
   *   <li>.startup_timer - Seconds since start began
   *   <li>.startup_progress - Progress 0.0-1.0 for bossbars
   *   <li>.startup_progress_percentage - Progress 0-100
   *   <li>.player_count - Number of players
   *   <li>.players - Collection of Player objects
   * </ul>
   */
  private Object resolveGlobalServerVariable(String variableName) {
    ...
    if (variableName.endsWith(".startup_timer")) {
      String serverName =
          variableName.substring(0, variableName.length() - ".startup_timer".length());
      if (serverManager.hasServer(serverName)) {
    ...
    if (variableName.endsWith(".startup_progress_percentage")) {
      String serverName =
          variableName.substring(
              0, variableName.length() - ".startup_progress_percentage".length());
      if (serverManager.hasServer(serverName)) {
    ...
    if (variableName.endsWith(".startup_progress")) {
      String serverName =
          variableName.substring(0, variableName.length() - ".startup_progress".length());
      if (serverManager.hasServer(serverName)) {
    ...
    if (variableName.endsWith(".player_count")) {
      String serverName =
          variableName.substring(0, variableName.length() - ".player_count".length());
      if (serverManager.hasServer(serverName)) {
