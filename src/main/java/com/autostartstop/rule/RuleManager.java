...
public class RuleManager {
  ...
  public void loadRules(PluginConfig config, RuleExecutor ruleExecutor) {
    ...
    for (Map.Entry<String, RuleConfig> entry : ruleConfigs.entrySet()) {
      ...
      try {
        ...
        if (!rule.isTemplateRule() && rule.getTriggers() != null) {
          for (Trigger trigger : rule.getTriggers()) {
            if (trigger.getType() == TriggerType.MANUAL
                && trigger instanceof ManualTrigger manualTrigger) {
        ...
        if (rule.isTemplateRule()) {
          logger.debug(
              "Rule '{}' loaded successfully (template: {})", name, ruleConfig.getTemplate());
        } else {
          int triggerCount = rule.getTriggers() != null ? rule.getTriggers().size() : 0;
          int actionCount = rule.getActions() != null ? rule.getActions().size() : 0;
          logger.debug(
              "Rule '{}' loaded successfully ({} triggers, {} actions)",
              name,
              triggerCount,
              actionCount);
        }
    ...
    logger.debug(
        "Loaded and activated {} rules ({} disabled, {} failed)",
        successCount,
        disabledCount,
        failCount);
  }
  ...
  private Rule createRule(String name, RuleConfig config) {
    ...
    if (config.isTemplateRule()) {
      logger.debug(
          "Rule '{}': creating template-based rule with template '{}'", name, config.getTemplate());
      TemplateConfig templateConfig = config.getTemplateConfig();
      if (templateConfig == null) {
        logger.error(
            "Rule '{}': template '{}' specified but template config is null",
            name,
            config.getTemplate());
    ...
    if (config.getTriggers() != null) {
      ...
      for (TriggerConfig triggerConfig : config.getTriggers()) {
        ...
        if (trigger != null) {
          ...
          logger.debug(
              "Rule '{}': trigger '{}' created successfully",
              name,
              trigger.getType().getConfigName());
        } else {
          logger.warn(
              "Rule '{}': failed to create trigger from config (type: {})",
              name,
              triggerConfig.getType());
        }
      }
    } else {
  ...
  /**
   * Gets all registered manual trigger IDs. Used by TriggerCommand for tab completion.
   *
  ...
  /**
   * Gets all manual triggers with the given ID. Used by TriggerCommand to fire manual triggers.
   *
