...
/**
 * Represents a rule that connects triggers to actions with optional conditions, or uses a template
 * for built-in behavior.
 */
public class Rule {
  ...
  /**
   * Activates all triggers in this rule, or the template if this is a template-based rule. Each
   * trigger will start listening for its respective events.
   *
   * @param executionCallback Callback to invoke when any trigger fires. Returns a CompletableFuture
   *     that completes when execution is done. Not used for template rules (templates handle their
   *     own execution).
   */
  ...
  /**
   * Deactivates all triggers in this rule, or the template if this is a template-based rule. Each
   * trigger will stop listening for events.
   */
