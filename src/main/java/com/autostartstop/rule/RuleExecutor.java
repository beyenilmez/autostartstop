...
public class RuleExecutor {
  ...
  /**
   * Executes a rule with the given context. Each execution is independent and runs in its own
   * thread.
   *
    ...
    return CompletableFuture.runAsync(
        () -> {
          ...
          try {
            ...
            logger.debug(
                "Rule '{}': beginning execution in thread {}",
                ruleName,
            ...
            if (rule.hasConditions()) {
              logger.debug(
                  "Rule '{}': evaluating {} conditions...",
                  ruleName,
                  rule.getConditions().getChecks() != null
                      ? rule.getConditions().getChecks().size()
                      : 0);
            ...
            if (actionConfigs != null && !actionConfigs.isEmpty()) {
              logger.debug(
                  "Rule '{}': executing {} actions sequentially", ruleName, actionConfigs.size());
              int actionIndex = 0;
              for (ActionConfig actionConfig : actionConfigs) {
                actionIndex++;
                logger.debug(
                    "Rule '{}': executing action {}/{} (type: {})",
                    ruleName,
                    actionIndex,
                    actionConfigs.size(),
            ...
            long duration = System.currentTimeMillis() - startTime;
            logger.error(
                "Rule '{}' execution failed after {}ms: {}", ruleName, duration, e.getMessage());
