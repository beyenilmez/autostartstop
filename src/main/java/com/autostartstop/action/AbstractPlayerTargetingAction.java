 ...
 * resolution logic and reduces boilerplate.
 *
 ...
 *
 * <ul>
 *   <li>player: Single player (Player object, player name, or UUID string)
 *   <li>players: Multiple players (list of player names/UUIDs)
 *   <li>server: Single server (target all players on that server)
 *   <li>servers: Multiple servers (target all players on those servers)
 * </ul>
 */
public abstract class AbstractPlayerTargetingAction implements Action {
  ...
  /**
   * Executes the action-specific logic for the resolved players. This method is called after
   * players have been resolved and validated.
   *
   * @param players The set of resolved players (never empty)
   * @param context The execution context
   * @param ruleName The name of the rule being executed
   */
  protected abstract void executeForPlayers(
      Set<Player> players, ExecutionContext context, String ruleName);
  ...
  public final CompletableFuture<Void> execute(ExecutionContext context) {
    ...
    Set<Player> targetPlayers =
        targetResolver.resolveTargetPlayers(
  ...
  /**
   * Gets the action name for logging purposes. Defaults to the config name from the action type.
   *
  ...
  public List<String> getPlayersParam() {
    return playersParam;
  }
  ...
  public List<String> getServersParam() {
    return serversParam;
  }
}
