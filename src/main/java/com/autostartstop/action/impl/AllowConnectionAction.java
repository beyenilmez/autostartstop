...
public class AllowConnectionAction implements Action {
  private static final Logger logger = PluginLogger.get(AllowConnectionAction.class);
  ...
  public AllowConnectionAction(
      String connectionParam, String serverParam, TargetResolver targetResolver) {
  ...
  public CompletableFuture<Void> execute(ExecutionContext context) {
    ...
    if (connectionObj == null) {
      logger.error(
          "({}) {}: No connection event found for '{}'", ruleName, ACTION_NAME, connectionParam);
      return CompletableFuture.completedFuture(null);
    }

    if (!(connectionObj instanceof ServerPreConnectEvent)) {
      logger.error(
          "({}) {}: '{}' is not a ServerPreConnectEvent: {}",
          ruleName,
          ACTION_NAME,
          connectionParam,
          connectionObj.getClass().getName());
      return CompletableFuture.completedFuture(null);
    }
    ...
    logger.debug(
        "({}) {}: Allowed '{}' to server '{}'", ruleName, ACTION_NAME, playerName, serverName);
