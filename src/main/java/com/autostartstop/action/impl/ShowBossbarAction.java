...
public class ShowBossbarAction extends AbstractPlayerTargetingAction {
  private static final Logger logger = PluginLogger.get(ShowBossbarAction.class);
  ...
  protected void executeForPlayers(Set<Player> players, ExecutionContext context, String ruleName) {
    ...
    logger.debug(
        "({}) {}: Showed '{}' to {} player(s)", ruleName, getActionName(), resolvedId, shownCount);
  }
  ...
  public String getOverlayRaw() {
    return overlayRaw;
  }

  public String getProgressRaw() {
    return progressRaw;
  }
}
