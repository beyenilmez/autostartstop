...
/**
 * Action that starts a server. Calls beginStartup() on the tracker which handles all monitoring
 * automatically.
 */
public class StartAction implements Action {
  private static final Logger logger = PluginLogger.get(StartAction.class);
  ...
  public static StartAction create(ActionConfig config, ActionContext ctx) {
    ...
    return new StartAction(
        server, ctx.serverManager(), ctx.variableResolver(), ctx.startupTracker());
  }
  ...
  public CompletableFuture<Void> execute(ExecutionContext context) {
    ...
    return serverManager
        ...
        .exceptionally(
            throwable -> {
              logger.error(
                  "Server '{}' start threw exception: {}", resolvedServer, throwable.getMessage());
              return null;
            });
  }
