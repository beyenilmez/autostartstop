...
public class WhileAction implements Action {
  ...
  public CompletableFuture<Void> execute(ExecutionContext context) {
    ...
    Duration updateInterval =
        variableResolver.resolveDuration(updateIntervalRaw, context, DEFAULT_UPDATE_INTERVAL);

    logger.debug(
        "({}) {}: starting loop (timeout: {}, interval: {})",
        ruleName,
        ACTION_NAME,
    ...
    return CompletableFuture.runAsync(
        () -> {
          ...
          long timeoutMs =
              finalTimeout != null && finalTimeout.toMillis() > 0
                  ? finalTimeout.toMillis()
                  : Long.MAX_VALUE;
          int iterations = 0;
          ...
          try {
            while (true) {
              ...
              if (elapsed >= timeoutMs) {
                logger.debug(
                    "({}) {}: timeout reached after {} iterations ({}ms)",
                    ruleName,
                    ACTION_NAME,
                    iterations,
                    elapsed);
              ...
              if (!conditionEvaluator.evaluate(conditionConfig, context)) {
                logger.debug(
                    "({}) {}: conditions no longer met after {} iterations ({}ms)",
                    ruleName,
                    ACTION_NAME,
                    iterations,
                    elapsed);
              ...
              if (doActions != null && !doActions.isEmpty()) {
                for (ActionConfig actionConfig : doActions) {
                  ...
                  if (remainingMs <= 0) {
                    logger.debug(
                        "({}) {}: timeout reached during action execution after {} iterations",
                        ruleName,
                        ACTION_NAME,
                        iterations);
                    timedOut = true;
                  ...
                  if (action != null) {
                    try {
                      ...
                      logger.debug(
                          "({}) {}: timeout reached while waiting for action '{}' after {} iterations",
                          ruleName,
                          ACTION_NAME,
              ...
              // Calculate how long this iteration took and wait only the remaining time to meet
              // update interval
              ...
              if (remainingIntervalTime > 0) {
                ...
                if (remainingTimeout <= 0) {
                  logger.debug(
                      "({}) {}: timeout reached before wait after {} iterations",
                      ruleName,
                      ACTION_NAME,
                      iterations);
                  break;
  ...
  public String getUpdateIntervalRaw() {
    return updateIntervalRaw;
  }
  ...
  public List<ActionConfig> getDoActions() {
    return doActions;
  }
}
