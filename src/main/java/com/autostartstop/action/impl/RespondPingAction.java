 ...
 * ProxyPingEvent to customize the ping response.
 *
 * <p>Configuration: - ping: ServerPing object to use (default: ${ping}, which means use
 * event.getPing()) - use_cached_motd: Use cached MOTD if true and cache exists (default: false) -
 * use_backend_motd: Use live backend server MOTD if true (used when cached MOTD not used or not
 * found) - motd: Custom MOTD in MiniMessage format (optional, used when cached/backend MOTD not
 * used) - version_name: Version name to set (optional) - protocol_version: Protocol version to set
 * (optional, as string like "-1") - icon: Server icon file path or base64 string (optional, must be
 * 64x64 PNG)
 */
public class RespondPingAction implements Action {
  private static final Logger logger = PluginLogger.get(RespondPingAction.class);
  ...
  public RespondPingAction(
      String pingParam,
      Boolean useCachedMotd,
      Boolean useBackendMotd,
      String motdParam,
      String versionName,
  ...
  public static RespondPingAction create(ActionConfig config, ActionContext ctx) {
    // ping parameter (ServerPing object to use, defaults to ${ping} which means use
    // event.getPing())
  ...
  public CompletableFuture<Void> execute(ExecutionContext context) {
    ...
    if (!(pingEventObj instanceof ProxyPingEvent)) {
      logger.error(
          "({}) {}: '{}' is not a ProxyPingEvent: {}",
          ruleName,
          ACTION_NAME,
          pingEventVar,
          pingEventObj.getClass().getName());
      return CompletableFuture.completedFuture(null);
    }
    ...
    if (useCachedMotd != null && useCachedMotd && motdCacheManager != null) {
      ...
      if (cachedMotd != null && !cachedMotd.isEmpty()) {
        ...
        logger.debug(
            "({}) {}: Using cached MOTD for virtual host '{}'", ruleName, ACTION_NAME, virtualHost);
      } else {
        logger.debug(
            "({}) {}: Cached MOTD requested but not found for virtual host '{}'",
            ruleName,
            ACTION_NAME,
            virtualHost);
      }
    ...
    if (motdComponent == null
      ...
      if (serverName != null) {
        ...
        if (registeredServer != null) {
          try {
            ...
            if (description != null) {
              ...
              logger.debug(
                  "({}) {}: Using backend MOTD from server '{}'",
                  ruleName,
                  ACTION_NAME,
                  serverName);
            }
            ...
            logger.warn(
                "({}) {}: Failed to get backend MOTD from server '{}': {}",
                ruleName,
                ACTION_NAME,
                serverName,
                e.getMessage());
          ...
          logger.debug(
              "({}) {}: No RegisteredServer for '{}', skipping backend MOTD",
              ruleName,
              ACTION_NAME,
              serverName);
        }
        ...
        logger.debug(
            "({}) {}: use_backend_motd true but ping.server not in context", ruleName, ACTION_NAME);
      }
    }
    ...
    if (originalVersion != null) {
      ...
      if (protocolVersion != null && !protocolVersion.isEmpty()) {
        try {
          ...
          logger.warn(
              "({}) {}: Invalid protocol_version '{}', using original protocol {}",
              ruleName,
              ACTION_NAME,
              protocolVersion,
              originalVersion.getProtocol());
        }
      }
      ...
      if (protocolVersion != null && !protocolVersion.isEmpty()) {
        try {
          ...
          logger.warn(
              "({}) {}: Invalid protocol_version '{}', using player protocol {}",
              ruleName,
              ACTION_NAME,
              protocolVersion,
              protocolToUse);
      ...
      String versionNameToUse =
          versionName != null
              ? MiniMessageUtil.toLegacy(variableResolver.resolve(versionName, context))
              : "Unknown";
      ServerPing.Version newVersion = new ServerPing.Version(protocolToUse, versionNameToUse);
      pingBuilder.version(newVersion);
      logger.debug(
          "({}) {}: Created version name='{}', protocol={}",
          ruleName,
          ACTION_NAME,
          versionNameToUse,
          protocolToUse);
    ...
    if (iconParam != null && !iconParam.isEmpty()) {
      ...
      if (iconImage != null) {
        ...
        logger.warn("({}) {}: Failed to load icon from '{}'", ruleName, ACTION_NAME, resolvedIcon);
      }
    }
