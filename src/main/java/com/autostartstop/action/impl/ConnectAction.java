...
public class ConnectAction implements Action {
  private static final Logger logger = PluginLogger.get(ConnectAction.class);
  ...
  public CompletableFuture<Void> execute(ExecutionContext context) {
    ...
    Set<Player> playersToConnect =
        targetResolver.resolvePlayersOnly(playerParam, playersParam, context, ACTION_NAME);
            ...
            () -> {
              long successCount =
                  futures.stream()
                      ...
                      .count();
              logger.debug(
                  "({}) {}: Connected {}/{} player(s) to '{}'",
                  ruleName,
                  ACTION_NAME,
                  successCount,
                  playersToConnect.size(),
                  serverName);
            });
  }

  private void sendConnectionErrorMessage(
      Player player, String serverName, String reasonText, ExecutionContext context) {
    ...
    try {
      ...
      logger.debug(
          ...
          context.getVariable("_rule_name", "unknown"),
          ACTION_NAME,
          player.getUsername(),
          e.getMessage());
    }
