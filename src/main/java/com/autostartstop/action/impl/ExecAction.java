  ...
  public CompletableFuture<Void> execute(ExecutionContext context) {
    ...
    if (environment != null && !environment.isEmpty()) {
      ...
      for (Map.Entry<String, String> entry : environment.entrySet()) {
        resolvedEnvironment.put(
            entry.getKey(), variableResolver.resolve(entry.getValue(), context));
      }
    }

    Duration resolvedTimeout =
        variableResolver.resolveDuration(timeout, context, CommandExecutor.DEFAULT_COMMAND_TIMEOUT);
