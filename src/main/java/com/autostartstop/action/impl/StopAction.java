...
public class StopAction implements Action {
  ...
  public static StopAction create(ActionConfig config, ActionContext ctx) {
    ...
    return new StopAction(
        server, ctx.serverManager(), ctx.variableResolver(), ctx.motdCacheManager());
  }
    ...
    return serverManager
        ...
        .thenAccept(
            success -> {
              ...
              if (success) {
                ...
                logger.warn(
                    "StopAction: server '{}' stop returned failure (duration: {}ms)",
                    resolvedServer,
                    duration);
              }
            })
        .exceptionally(
            throwable -> {
              long duration = System.currentTimeMillis() - startTime;
              logger.error(
                  "StopAction: server '{}' stop threw exception after {}ms: {}",
                  resolvedServer,
                  duration,
