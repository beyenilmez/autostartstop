...
public class IfAction implements Action {
  ...
  public static class ElseIfBranch {
    ...
    public List<ActionConfig> getThenActions() {
      return thenActions;
    }
  }

  public IfAction(
      ConditionConfig primaryCondition,
      List<ActionConfig> thenActions,
      List<ElseIfBranch> elseIfBranches,
      List<ActionConfig> elseActions,
        ...
        () -> {
          try {
            if (conditionEvaluator.evaluate(primaryCondition, context)) {
              logger.debug(
                  "({}) {}: primary condition TRUE, executing 'then' actions",
                  ruleName,
                  ACTION_NAME);
            ...
            logger.debug(
                "({}) {}: primary condition FALSE, checking else_if branches",
                ruleName,
                ACTION_NAME);
            ...
            if (elseIfBranches != null && !elseIfBranches.isEmpty()) {
              for (int i = 0; i < elseIfBranches.size(); i++) {
                ...
                if (conditionEvaluator.evaluate(branch.getCondition(), context)) {
                  logger.debug(
                      "({}) {}: else_if branch {} TRUE, executing 'then' actions",
                      ruleName,
                      ACTION_NAME,
                      i + 1);
            ...
            if (elseActions != null && !elseActions.isEmpty()) {
              logger.debug(
                  "({}) {}: no conditions matched, executing 'else' actions",
                  ruleName,
                  ACTION_NAME);
              executeActions(elseActions, context, ruleName);
            } else {
              logger.debug(
                  "({}) {}: no conditions matched and no 'else' actions defined",
                  ruleName,
                  ACTION_NAME);
            }
            ...
            logger.error(
                "({}) {}: error during conditional execution: {}",
                ruleName,
                ACTION_NAME,
                e.getMessage());
          }
  ...
  private void executeActions(
      List<ActionConfig> actions, ExecutionContext context, String ruleName) {
  ...
  public ConditionConfig getPrimaryCondition() {
