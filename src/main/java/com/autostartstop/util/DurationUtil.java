 ...
 * (ticks), 500ms, 5s, 2m, 1h If no unit is specified, the value is interpreted as milliseconds
...
public class DurationUtil {
   ...
   *
   * @param input The duration string (e.g., "10t", "500ms", "5s", "2m", "1h", or "500" for
   *     milliseconds)
  ...
  public static Duration parse(String input) {
    ...
    if (!matcher.matches()) {
      logger.error(
          "DurationParser: invalid format '{}' - expected format: <number> or <number><unit> (t, ms, s, m, h)",
          input);
