 ...
 * common resolution logic used across player/server targeting actions.
 *
 * <p>Supports resolving:
 *
 * <ul>
 *   <li>Single player from 'player' parameter
 *   <li>Multiple players from 'players' parameter (list or variable reference)
 *   <li>Single server from 'server' parameter
 *   <li>Multiple servers from 'servers' parameter (list or variable reference)
 *   <li>Players connected to resolved servers
 * </ul>
 *
 * <p>Player resolution supports:
 *
 * <ul>
 *   <li>Player objects directly
 *   <li>Player names (String)
 *   <li>Player UUIDs (String format)
 *   <li>Collections of players or names
...
public class TargetResolver {
  ...
  public Set<Player> resolvePlayersOnly(
      String playerParam, List<String> playersParam, ExecutionContext context, String actionName) {
   ...
   */
  public RegisteredServer resolveServer(
      String serverParam, ExecutionContext context, String actionName) {
    if (serverParam == null || serverParam.isBlank()) {
      return null;
    }
    ...
    if (serverParam.startsWith("${") && serverParam.endsWith("}")) {
      ...
      if (serverObj != null) {
        ...
        logger.warn(
            "({}) {}: Variable '{}' is not a server: {}",
            ruleName,
            actionName,
            variableName,
            serverObj.getClass().getSimpleName());
      }
      return null;
    }
  ...
  /**
   * Resolves a player parameter and adds the resulting player(s) to the collection. Handles Player
   * objects, player name strings, and Collections. Only resolves as object variable if wrapped in
   * ${...}.
   */
  private void resolveAndAddPlayers(
      String param,
      ExecutionContext context,
    ...
    if (param.startsWith("${") && param.endsWith("}")) {
      ...
      if (resolved != null) {
        if (resolved instanceof Player) {
          ...
          logger.debug(
              "({}) {}: Resolved variable '{}' to Player", ruleName, actionName, variableName);
          return;
   ...
   */
  private boolean resolvePlayerByName(
      String playerIdentifier, Collection<Player> players, String actionName, String ruleName) {
    ...
    logger.warn(
        "({}) {}: Player '{}' not found or not online", ruleName, actionName, playerIdentifier);
    return false;
  }

  /** Resolves a server parameter and adds all players on that server to the collection. */
  private void resolveAndAddServerPlayers(
      String param,
      ExecutionContext context,
      Collection<Player> players,
      String actionName,
  ...
  private RegisteredServer resolveServerByName(
      String serverName, String actionName, String ruleName) {
