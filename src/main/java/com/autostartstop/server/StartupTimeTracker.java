...
public class StartupTimeTracker {
  ...
  /**
   * Gets the calculated expected startup time for a server based on historical data. Uses a
   * weighted moving average with recent times having more weight.
   *
   * @param serverName The server name
  ...
  public Duration getExpectedStartupTime(String serverName) {
    ...
    if (times == null || times.isEmpty()) {
      logger.debug(
          "No startup time data for server '{}', using default {}s",
          serverName,
          DEFAULT_EXPECTED_TIME.getSeconds());
    ...
    synchronized (times) {
      ...
      logger.debug(
          "Calculated expected startup time for '{}': {}ms (from {} samples)",
          serverName,
          avgMs,
          size);
