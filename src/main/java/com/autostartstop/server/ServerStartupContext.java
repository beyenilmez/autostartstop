...
/**
 * Tracks the startup state of a server during an active startup operation. Provides dynamic values
 * for startup_timer, startup_progress_percentage, and state.
 */
public class ServerStartupContext {
  ...
  public ServerStartupContext(String serverName, Duration expectedStartupTime) {
    ...
    this.expectedStartupTime =
        expectedStartupTime != null ? expectedStartupTime : Duration.ofSeconds(30);
  ...
  /** Gets the startup timer in seconds. This is calculated dynamically from the start time. */
  ...
  /**
   * Gets the startup progress percentage (0-100). This is calculated from the timer and expected
   * startup time.
   */
  public int getStartupProgressPercentage() {
    ...
    if (ServerState.OFFLINE.getName().equals(currentState)
        || ServerState.FAILED.getName().equals(currentState)) {
      return 0;
    }
  ...
  /**
   * Gets the startup progress as a value between 0.0 and 1.0. This is useful for bossbars which
   * expect progress in this range.
   */
