...
public class ManagedServer {
  ...
  public CompletableFuture<Boolean> start(boolean force) {
    if (controlApi == null) {
      logger.warn(
          "Cannot start server '{}': control API not available (check debug logs for errors)",
          name);
    ...
    logger.debug(
        "Server '{}': delegating start to control API (type: {}){}",
        name,
        controlApi.getType(),
        force ? " [forced]" : "");
    return controlApi
  ...
  public CompletableFuture<Boolean> stop() {
    if (controlApi == null) {
      logger.warn(
          "Cannot stop server '{}': control API not available (check debug logs for errors)", name);
      return CompletableFuture.completedFuture(false);
    }
    logger.debug(
        "Server '{}': delegating stop to control API (type: {})", name, controlApi.getType());
    return controlApi
        .stop()
        .thenApply(
            result -> {
  ...
  public CompletableFuture<Boolean> restart() {
    if (controlApi == null) {
      logger.warn(
          "Cannot restart server '{}': control API not available (check debug logs for errors)",
          name);
      return CompletableFuture.completedFuture(false);
    }
    logger.debug(
        "Server '{}': delegating restart to control API (type: {})", name, controlApi.getType());
    return controlApi
        .restart()
        .thenApply(
            result -> {
        ...
        .exceptionally(
            e -> {
              logger.error(
                  "Server '{}': restart command threw exception: {}", name, e.getMessage());
  ...
  public CompletableFuture<Boolean> sendCommand(String command) {
    if (controlApi == null) {
      logger.warn(
          "Cannot send command to server '{}': control API not available (check debug logs for errors)",
          name);
      return CompletableFuture.completedFuture(false);
    }
    ...
    if (!controlApi.supportsCommandSending()) {
      logger.warn(
          "Cannot send command to server '{}': control API (type: {}) does not support command sending",
          name,
          controlApi.getType());
      return CompletableFuture.completedFuture(false);
    }
    ...
    logger.debug(
        "Server '{}': delegating command send to control API (type: {}): {}",
        name,
        controlApi.getType(),
        command);
    return controlApi
    ...
    if (pingConfig != null) {
      ...
      if (timeout != null && !timeout.isBlank()) {
        try {
          ...
          logger.warn(
              "Server '{}': invalid ping.timeout '{}', using default 30s. Error: {}",
              name,
              timeout,
              e.getMessage());
        }
  ...
  /**
   * Checks if this server is online by pinging it. Uses the configured ping_method setting, falling
   * back to velocity ping if control_api ping is not supported.
   *
  ...
  public boolean isOnline() {
    ...
    if ("control_api".equals(pingMethod)) {
      if (controlApi != null && controlApi.supportsPing()) {
        logger.debug(
            "Pinging server '{}' using control_api (type: {})", name, controlApi.getType());
        try {
          ...
          return controlApi
              ...
              .handle(
                  (online, throwable) -> {
                    if (throwable != null) {
                      logger.debug(
                          "Server '{}' control_api ping failed: {}", name, throwable.getMessage());
                      return false;
                    }
                    logger.debug(
                        "Server '{}' control_api ping result: {}",
                        name,
        ...
        logger.debug(
            "Server '{}': control_api ping requested but not supported (type: {}), falling back to velocity ping",
            name,
            controlApi != null ? controlApi.getType() : "null");
      }
    }
  ...
  /**
   * Gets the current state of this server. Returns normalized ServerState enum value. For AMP:
   * Returns normalized ApplicationState from control API. For Shell: Returns normalized state based
   * on ping (starting state is managed by ServerStartupTracker)
   *
