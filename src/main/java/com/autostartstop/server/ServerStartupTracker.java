 ...
 * timer, and progress for variable resolution.
 *
 * <p>When beginStartup() is called, a background monitor automatically polls the server and
...
public class ServerStartupTracker {
  ...
  /**
   * Begins tracking a server startup and starts a background monitor. The monitor will
   * automatically complete tracking when the server comes online.
   *
  ...
  public ServerStartupContext beginStartup(String serverName) {
    ...
    logger.debug(
        "Began tracking startup for '{}' (expected: {}ms)", serverName, expectedTime.toMillis());
  ...
  /**
   * Starts a background task that monitors the server and completes tracking when the server comes
   * online or times out.
   */
        ...
        () -> {
          ...
          while (true) {
            ...
            if (context == null || context.isCompleted()) {
              logger.debug(
                  "ServerStartupTracker: monitor exiting for '{}' - tracking completed",
                  serverName);
              return;
            }
            ...
            if (System.currentTimeMillis() - startTime > MONITOR_TIMEOUT_MS) {
              logger.warn(
                  "ServerStartupTracker: monitor timeout for '{}' after {}ms",
                  serverName,
                  MONITOR_TIMEOUT_MS);
  ...
  public void completeStartup(String serverName, boolean success) {
    ...
    logger.debug(
        "Completed startup tracking for '{}' (success: {}, duration: {}ms)",
        serverName,
        success,
        context.getStartupTimerMs());
  }
  ...
  /**
   * Gets the current state for a server. Returns normalized ServerState enum value. For AMP:
   * Returns normalized ApplicationState from control API. For Shell: Returns STARTING if active
   * tracking, otherwise ONLINE/OFFLINE based on ping.
   *
  ...
  /** Gets the startup timer in seconds for a server. Returns 0 if no active startup. */
  ...
  /** Gets the startup progress (0.0-1.0) for a server. Useful for bossbars. */
  ...
  /**
   * Gets the expected startup time for a server. Priority: auto-calculated (if enabled and has
  ...
  private Duration getExpectedStartupTime(String serverName) {
    ...
    if (timerConfig == null) {
      ...
      if (startupTimeTracker.hasData(serverName)) {
        ...
        logger.debug(
            "Using auto-calculated startup time for '{}': {}ms", serverName, autoCalc.toMillis());
        return autoCalc;
      }
      return DEFAULT_EXPECTED_TIME;
    }
    ...
    if (timerConfig.isAutoCalculateExpectedStartupTime()
        && startupTimeTracker.hasData(serverName)) {
      Duration autoCalc = startupTimeTracker.getExpectedStartupTime(serverName);
      logger.debug(
          "Using auto-calculated startup time for '{}': {}ms", serverName, autoCalc.toMillis());
      return autoCalc;
    ...
    if (expectedTimeStr != null && !expectedTimeStr.isBlank()) {
      try {
        ...
        logger.warn(
            "Invalid expected_startup_time '{}' for server '{}', using default",
            expectedTimeStr,
            serverName);
      }
    }
