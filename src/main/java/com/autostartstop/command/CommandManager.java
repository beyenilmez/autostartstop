...
public class CommandManager implements SimpleCommand {
  private static final Logger logger = PluginLogger.get(CommandManager.class);
  ...
  public void register() {
    ...
    commandManager.register(
        commandManager.metaBuilder(MAIN_COMMAND).aliases(ALIAS).plugin(plugin).build(), this);
  ...
  public void execute(Invocation invocation) {
    ...
    if (subCommand == null) {
      ...
      source.sendMessage(
          MiniMessageUtil.parse("<red>Unknown subcommand: " + subCommandName + "</red>"));
      showHelp(source);
      return;
    }
    ...
    logger.debug(
        "Executing subcommand '{}' for {} with args: {}",
        subCommandName,
        source,
        Arrays.toString(subArgs));
