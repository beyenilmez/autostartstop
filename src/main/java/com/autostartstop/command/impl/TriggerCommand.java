...
public class TriggerCommand implements SubCommand {
  ...
  public void execute(CommandSource source, String[] args) {
    ...
    if (matchingTriggers.isEmpty()) {
      logger.warn(
          "TriggerCommand: no manual triggers found for ID '{}' (requested by {})",
          triggerId,
          source);
      source.sendMessage(
          MiniMessageUtil.parse("<red>No manual triggers found with ID: " + triggerId + "</red>"));
      return;
    }
    ...
    if (activatedCount == 0) {
      logger.warn(
          "TriggerCommand: found {} manual triggers for ID '{}' but none are activated",
          matchingTriggers.size(),
          triggerId);
      source.sendMessage(
          MiniMessageUtil.parse(
              "<red>Manual triggers with ID '"
                  + triggerId
                  + "' exist but are not activated.</red>"));
      return;
    }

    logger.debug(
        "Manual trigger '{}' fired by {} with args: {} ({} triggers)",
        triggerId,
        source,
        Arrays.toString(triggerArgs),
        activatedCount);
    ...
    source.sendMessage(
        MiniMessageUtil.parse(
            "<green>Fired " + fired + " manual trigger(s) with ID '" + triggerId + "'.</green>"));
