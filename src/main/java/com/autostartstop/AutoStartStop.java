...
public class AutoStartStop {
  ...
  public void onProxyShutdown(ProxyShutdownEvent event) {
    ...
    if (pluginConfig != null && pluginConfig.getSettings() != null) {
      try {
        ...
        logger.debug(
            "Using configured shutdown timeout: {}",
            pluginConfig.getSettings().getShutdownTimeout());
      } catch (Exception e) {
  ...
  /**
   * Registers trigger creators via TriggerContext. All triggers now use static create methods
   * registered in TriggerType. Note: TriggerContext will be updated with actual settings after
   * config is loaded.
   */
  ...
  /**
   * Registers action creators via ActionContext. All actions now use static create methods
   * registered in ActionType. Note: ActionContext will be updated with actual settings after config
   * is loaded.
   */
  ...
  /**
   * Registers condition creators via ConditionContext. All conditions now use static create methods
   * registered in ConditionType. Note: ConditionContext will be updated with actual settings after
   * config is loaded.
   */
  ...
  /**
   * Registers template creators via TemplateContext. All templates use static create methods
   * registered in TemplateType. Note: TemplateContext will be updated with actual settings after
   * config is loaded.
   */
  ...
  /**
   * Registers control API creators. All control APIs now use static create methods registered in
   * ServerControlApiType.
   */
  ...
  /**
   * Initializes bStats metrics. bStats respects its own global configuration in
   * plugins/bStats/config.txt
   */
  ...
  private void initializeMotdCacheManager() {
    ...
    try {
      ...
      logger.debug(
          "MOTD cache manager initialized with interval: {}", DurationUtil.format(cacheInterval));
    } catch (Exception e) {
   ...
   * Called after config is loaded/reloaded to ensure all contexts have access to the actual
   * settings.
   *
  ...
  /** Reloads the plugin configuration. Thread-safe: synchronized to prevent concurrent reloads. */
  public void reloadConfiguration() {
    synchronized (reloadLock) {
      ...
      try {
        ...
        // Update all contexts with reloaded settings (isReload=true to prevent proxy_start from
        // firing again)
        ...
        logger.info(
            "Configuration reloaded (version: {}, servers: {}, rules: {})",
