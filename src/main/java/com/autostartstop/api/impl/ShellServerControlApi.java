...
public class ShellServerControlApi implements ServerControlApi {
  private static final Logger logger = PluginLogger.get(ShellServerControlApi.class);
  ...
  public ShellServerControlApi(
      String serverName,
      String startCommand,
      String stopCommand,
      String restartCommand,
      String sendCommandCommand,
    ...
    this.commandTimeout =
        commandTimeout != null ? commandTimeout : CommandExecutor.DEFAULT_COMMAND_TIMEOUT;
    ...
    logger.debug(
        ...
        serverName,
        startCommand != null ? "[configured]" : "[not set]");
    logger.debug(
        "Server '{}': stop_command={}",
        serverName,
        stopCommand != null ? "[configured]" : "[not set]");
    logger.debug(
        "Server '{}': restart_command={}",
        serverName,
        restartCommand != null ? "[configured]" : "[not set]");
    logger.debug(
        "Server '{}': send_command_command={}",
        serverName,
        sendCommandCommand != null ? "[configured]" : "[not set]");
    logger.debug(
        "Server '{}': working_directory={}",
        serverName,
        workingDirectory != null ? workingDirectory : "[not set]");
    logger.debug("Server '{}': command_timeout={}s", serverName, this.commandTimeout.toSeconds());
    logger.debug(
        "Server '{}': environment={} variable(s)",
        serverName,
        environment != null ? environment.size() : 0);
  }
  ...
  public CompletableFuture<Boolean> sendCommand(String command) {
    ...
    if (sendCommandCommand == null || sendCommandCommand.isBlank()) {
      logger.warn(
          "Server '{}': cannot send command - no send_command_command configured", serverName);
      return CompletableFuture.completedFuture(false);
    }
