...
/**
 * Interface for server control APIs. Implementations handle starting, stopping, and restarting
 * servers through various control mechanisms (shell commands, AMP, etc.).
 */
public interface ServerControlApi {
  ...
  /**
   * Pings the server to check if it's online using the control API. Only call this if
   * supportsPing() returns true.
   *
  ...
  /**
   * Gets the current state of the server. Returns a normalized ServerState enum value.
   *
  ...
  /**
   * Sends a command to the server console. Only call this if supportsCommandSending() returns true.
   *
