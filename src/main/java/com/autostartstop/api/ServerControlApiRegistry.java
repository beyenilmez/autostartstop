 ...
 * create methods defined in ServerControlApiType.
 */
public class ServerControlApiRegistry {
  ...
  public ServerControlApi create(ControlApiConfig config, String serverName) {
    ...
    if (type == null) {
      logger.error(
          "Server '{}': unknown API type '{}' - valid types: {}",
          serverName,
          configType,
          ServerControlApiType.getValidNames());
      return null;
    }
    ...
    try {
      if (!type.hasCreator()) {
        logger.error(
            "Server '{}': API type '{}' has no creator defined", serverName, type.getConfigName());
        return null;
      }

      logger.debug(
          "Server '{}': invoking creator to create '{}' control API", serverName, configType);
      ServerControlApi api = type.getCreator().create(config, serverName);
      if (api != null) {
        logger.debug(
            "Server '{}': control API created successfully (type: {})", serverName, api.getType());
      } else {
        logger.error(
            "Server '{}': creator returned null for API type '{}'", serverName, configType);
      }
      return api;
    } catch (Exception e) {
