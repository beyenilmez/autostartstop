...
public class ConditionEvaluator {
  private static final Logger logger = PluginLogger.get(ConditionEvaluator.class);
  ...
  public boolean evaluate(ConditionConfig config, ExecutionContext context) {
    ...
    for (Map<String, Object> checkMap : checks) {
      ...
      if (invert) {
        result = !result;
        logger.debug(
            "ConditionEvaluator: condition '{}' evaluated to {} (inverted)",
            condition.getType().getConfigName(),
            result ? "TRUE" : "FALSE");
      } else {
        logger.debug(
            "ConditionEvaluator: condition '{}' evaluated to {}",
            condition.getType().getConfigName(),
            result ? "TRUE" : "FALSE");
      }
    ...
    logger.debug(
        "ConditionEvaluator: evaluation complete - passed: {}, failed: {}, skipped: {}, result: {}",
        passCount,
        failCount,
        skipCount,
        finalResult);
  ...
  /**
   * Checks if a condition should have its result inverted. Looks for an "invert" key in the
